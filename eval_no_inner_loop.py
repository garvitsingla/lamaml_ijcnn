...
p.add_argument("--num-dists", type=int, default=8)
...
for i in range(N_MISSIONS):
    ...
    # 2. MAML (no adaptation): use θ directly (no inner-loop gradient)
    maml_zero_steps = []
    for ep in range(N_EPISODES):
        env.reset_task(mission)
    ...
    # 3. MAML (k-shot adaptation): inner-loop gradients with few examples
