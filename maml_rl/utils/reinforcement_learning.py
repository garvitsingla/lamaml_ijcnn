...
def value_iteration(transitions, rewards, gamma=0.95, theta=1e-5):
    rewards = np.expand_dims(rewards, axis=2)  # shape = (num_states, num_actions, 1)
    values = np.zeros(transitions.shape[0], dtype=np.float32)  #state values initialized to 0 shape = (num_states,)
    delta = np.inf
    while delta >= theta:
        q_values = np.sum(transitions * (rewards + gamma * values), axis=2)  # column wise sum to get q values of indivial state action pair
        new_values = np.max(q_values, axis=1) #row wise finding max of action values 
        delta = np.max(np.abs(new_values - values)) #max of all state's absolute difference of new values and old values (n',1) - (n,1)
        values = new_values
