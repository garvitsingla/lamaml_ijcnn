...
# Adapter
mission_adapter = MissionParamAdapter(mission_encoder.output_dim, policy_param_shapes).to(device)
...
# restore saved lang-adapted policy (ablation baseline - BERT without meta-learning)
...
def get_language_adapted_params(policy, mission_str, mission_encoder, mission_adapter, device):
    with torch.no_grad():
        mission_emb = mission_encoder(mission_str).to(device)   # [1, d]
...
print("Comparing language-adapted policy and language conditioned policy")
for i in range(N_MISSIONS):
    ...
    results_ablation.append(mean_ablation)


# Results
print("\n===== FINAL AGGREGATE RESULTS =====")
print(f"LA-MAML policy: {np.mean(results_lang):.2f} ± {np.std(results_lang):.2f}")
print(f"language-conditioned policy:   {np.mean(results_ablation):.2f} ± {np.std(results_ablation):.2f}")
